...
- **Scripted-policy renders (NOT brain-driven).** `renders/episode_greedy.mp4`, and the `staged_*` files (`staged_demo.mp4`, `staged_overview.png`, `staged_reels.png`, `staged_smoking.png`) — choreographed/scripted animation for illustration, not output of the connectome model. A further staged choreography clip, `renders/staged_skip_food.mp4`, is being added (also staged animation, not model output — not yet present in this checkout).
